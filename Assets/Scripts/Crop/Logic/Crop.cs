...
namespace Farm.CropPlant
{
    public class Crop : MonoBehaviour
    {
        ...
        public void SpawnHarvestItems()
        {
                ...
                for (int j = 0; j < amountToProduce; j++)
                {
                    ...
                    else
                    {
                        ...
                        var spawnPos = new Vector3(
                            transform.position.x + Random.Range(dirX, cropDetails.spawnRaduis.x * dirX),
                            transform.position.y + Random.Range(-cropDetails.spawnRaduis.y, cropDetails.spawnRaduis.y),
                            0);
            ...
            if (tileDetails != null)
            {
                int seedItemId = tileDetails.seedItemId;
                ...
                // 随机生成的物品的存在数量-1
                RandCropGenerator.Instance.DecrCurNum(seedItemId);

                // 随机生成物品
                RandCropGenerator.Instance.GenerateMap();
            }
            else
            {
